...
export const Hero: React.FC<HeroProps> = ({ onStart, lang = 'pt' as LanguageCode, setLang }) => {
  ...
  // Timer for Urgency (24 hours)
  const [timeLeft, setTimeLeft] = useState(23 * 60 * 60 + 45 * 60); // Start at 23h 45m to look realistic
  
  // Rotating Tag State
  const [currentTagIndex, setCurrentTagIndex] = useState(0);
  // Rotating Title State (NEW)
  ...
  // Cycle through tags every 3.5 seconds
  useEffect(() => {
  ...
  // Cycle through titles every 5 seconds (Slower for readability)
  useEffect(() => {
  ...
  return (
    <div className="relative overflow-hidden pb-24">
      ...
      {/* --- BACKGROUND IMAGE LAYER (CASCATA) --- */}
      <div className="absolute inset-0 z-0 select-none pointer-events-none">
          {/* Imagem de Fundo Visível agora */}
          <div className="absolute inset-0 bg-black/10 z-10"></div> {/* Darken slightly for text pop */}
          <img 
          ...
          />
          {/* Overlay Gradiente Suave (não cobre a imagem totalmente) */}
      ...
      {/* Cards Display - Visual Validation */}
      ...
      {/* Comparison Section - Why Choose CaloryIA */}
      <div className="mt-24 max-w-5xl mx-auto px-4 relative z-30 animate-fade-in-up delay-300">
        ...
        {/* --- MAIN CTA MOVED HERE AS REQUESTED --- */}
